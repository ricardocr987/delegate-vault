import { address, Address, getAddressEncoder, getProgramDerivedAddress, getU16Encoder } from '@solana/kit';
...
export async function getManagerAddress(authority: Address) {
    return (await getProgramDerivedAddress({
        programAddress: DELEGATE_VAULT_PROGRAM,
        seeds: [
            Buffer.from('manager'),
...
export async function getOrderVaultAddress(signer: Address, manager: Address, order: Address, token: Address) {
    return (await getProgramDerivedAddress({
        programAddress: DELEGATE_VAULT_PROGRAM,
        seeds: [
...
export async function getTokenVaultAddress(signer: Address, manager: Address, order: Address, token: Address) {
    return (await getProgramDerivedAddress({
        programAddress: DELEGATE_VAULT_PROGRAM,
        seeds: [
...
export async function getTickArrayAddress(whirlpool: Address, startTickIndex: number) {
    return (await getProgramDerivedAddress({
        programAddress: WHIRLPOOL_PROGRAM_ADDRESS,
        seeds: [Buffer.from('tick_array'), getAddressEncoder().encode(whirlpool), `${startTickIndex}`],
    }))[0];
}
...
export async function getAtaAddress(owner: Address, mint: Address, tokenProgram: Address = TOKEN_PROGRAM) {
...
export async function getPositionAddress(positionMint: Address) {
    return (await getProgramDerivedAddress({
        programAddress: WHIRLPOOL_PROGRAM_ADDRESS,
        seeds: [Buffer.from('position'), getAddressEncoder().encode(positionMint)],
    }))[0];
}

const whirlpoolConfigAddress = address("2LecshUwdy9xi7meFgHtFJQNSKk4KdTrcpvaB56dP2NQ");
