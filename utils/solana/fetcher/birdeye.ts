...
export type BirdeyeSearchResult = {
  name: string;
  symbol: string;
  address: string;
  decimals: number;
...
export type BirdeyeSearchResponse = {
  success: boolean;
  data: {
    items: Array<{
      type: string;
      result: BirdeyeSearchResult[];
    }>;
  };
};
...
export async function fetchTokensData(mintAddresses: string[]) {
  ...
  try {
    ...
    const { marketData, validMintAddresses } = await fetchMarketData(
      mintAddresses
    );
...
export async function searchTokens(keyword: string, limit: number = 1) {
  try {
    const response = await birdeyeClient
      .get("defi/v3/search", {
        searchParams: {
      ...
      .json<BirdeyeSearchResponse>();

    if (!response?.success || !response?.data?.items?.length) {
      return [];
    }

    return response.data.items[0]?.result || [];
  } catch (error) {
